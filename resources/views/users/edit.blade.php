...
<article class="page-container">
    <form action="/users_edit" method="post" class="form form-horizontal" id="form-admin-edit">
        <div class="row cl">
        ...
        <div class="row cl">
            ...
            <div class="formControls col-xs-8 col-sm-9">
                <input type="text" class="input-text" placeholder="用于找回密码" value="{{ $userinfo->email }}" name="email" id="email">
            </div>
        </div>
            ...
            <div class="formControls col-xs-8 col-sm-9"> <span class="select-box" style="width:150px;">
                <select class="select" name="role" id="role" size="1">
                    ...
                    @foreach($role_info as $val)
                        <option value="{{ $val->r_id }}" <?php if($userinfo->r_id == $val->r_id){ echo 'selected="selected"'; } ?>>{{ $val->r_name }}</option>
                    @endforeach
                </select>
                </span>
            </div>
    ...
    $(function(){
        $("#form-admin-edit").validate({
            ...
            submitHandler:function(form){
                ...
                // var role = $('#role').val();
                // var url = '/users_edit';
                // var data = {'name':name,'email':email,'password':password,'role':role,'id':id,'r_id':r_id};
