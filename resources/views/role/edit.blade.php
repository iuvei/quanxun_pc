...
<article class="page-container">
    <form method="post" action="/role_edit" enctype="multipart/form-data" class="form form-horizontal" id="form-domain-add">
        <div class="row cl">
            <div class="col-xs-8 col-sm-9 col-xs-offset-4 col-sm-offset-2">
                <p>角色名：<span style="color:red;">{{ $r_name }}</span><input type="hidden" value="{{ $r_id }}" id="r_id"></p>
                <div class="select_radio">
                    @foreach($power_info as $value)
                        <div class="list_one">
                            <div class="top"><input type="checkbox" name="p_id" class="top_checkbox" value="{{ $value['p_id'] }}" @if(in_array($value['p_id'], $i_id)) checked="checked" @endif>{{ $value['p_name'] }}----------</div>
                            @foreach($value['c_info'] as $val)
                                <div class="c_info"><input type="checkbox" name="p_id" class="c_info_checkbox" value="{{ $val['p_id'] }}" @if(in_array($val['p_id'], $i_id)) checked="checked" @endif>{{ $val['p_name'] }}----------</div>
                                <div class="z_info">
                                    @foreach($val['z_info'] as $z_val)
                                        <input class="zz_info" name="p_id" type="checkbox" value="{{ $z_val['p_id'] }}" @if(in_array($z_val['p_id'], $i_id)) checked="checked" @endif>{{ $z_val['p_name'] }}
                                    @endforeach
                                </div>
                            @endforeach
                        </div>
