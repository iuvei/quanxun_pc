...
<article class="page-container">
    <form method="post" action="/newsinformation_add" enctype="multipart/form-data" class="form form-horizontal" id="form-article-add">
        <div class="row cl">
            ...
            <div class="formControls col-xs-8 col-sm-9"> <span class="select-box">
				<select name="newstype" id="newstype" class="select">
                    ...
                    @foreach($newsinformation_type as $val)
                        <option value="{{ $val->nt_type }}">{{ $val->nt_type }}</option>
                    @endforeach
				</select>
				</span> </div>
        </div>
        ...
        {{--<div class="row cl">--}}
            ...
            {{--<div class="formControls col-xs-8 col-sm-9">--}}
                {{--<input type="text" onfocus="WdatePicker({ dateFmt:'yyyy-MM-dd HH:mm:ss',minDate:'#F{$dp.$D(\'commentdatemin\')||\'%y-%M-%d\'}' })" id="commentdatemax" name="commentdatemax" class="input-text Wdate">--}}
            {{--</div>--}}
        {{--</div>--}}
        <div class="row cl">
